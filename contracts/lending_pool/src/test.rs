...
fn test_deposit_event_emitted() {
    ...
    let (token_id, _stellar_asset_client, token_client) = create_token_contract(&env, &admin);
    let provider = Address::generate(&env);
    ...
    let event = &events[0];
    assert_eq!(event.topic[0], Symbol::new(&env, "Deposit"));
    assert_eq!(event.topic[1], provider);
    assert_eq!(event.topic[2], token_id);
    
    let (amount, shares) = event.data.clone().try_into::<(i128, i128)>().unwrap();
    assert_eq!(amount, 1000);
    assert_eq!(shares, 1000); // First depositor gets 1:1 shares
}
...
fn test_withdraw_event_emitted() {
    ...
    // Mint tokens to provider and deposit
    token_client.mint(&provider, &1000);
    pool_client.deposit(&provider, &token_id, &1000);
    
    // Clear events from deposit
    env.events().all();
    ...
    let event = &events[0];
    assert_eq!(event.topic[0], Symbol::new(&env, "Withdraw"));
    assert_eq!(event.topic[1], provider);
    assert_eq!(event.topic[2], token_id);
...
#[test]
fn test_deposit_cap_updated_event_emitted() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // Update deposit cap
    pool_client.set_max_pool_size(&token_id, &5000);
    ...
    let event = &events[0];
    assert_eq!(event.topic[0], Symbol::new(&env, "DepositCapUpdated"));
    assert_eq!(event.topic[1], token_id);
    
    let (old_cap, new_cap) = event.data.clone().try_into::<(i128, i128)>().unwrap();
    assert_eq!(old_cap, 0); // Default is 0 (unlimited)
    assert_eq!(new_cap, 5000);
}

#[test]
fn test_pool_pause_unpause_events_emitted() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let events = env.events().all();
    assert_eq!(events.len(), 1);
    assert_eq!(events[0].topic[0], Symbol::new(&env, "PoolPaused"));
    
    // Clear events and unpause
    env.events().all();
    pool_client.unpause();
    
    let events = env.events().all();
    assert_eq!(events.len(), 1);
    assert_eq!(events[0].topic[0], Symbol::new(&env, "PoolUnpaused"));
}
...
fn test_admin_transfer_events_emitted() {
    ...
    let events = env.events().all();
    assert_eq!(events.len(), 1);
    assert_eq!(events[0].topic[0], Symbol::new(&env, "AdminProposed"));
    assert_eq!(events[0].topic[1], admin);
    assert_eq!(events[0].data.try_into::<Address>().unwrap(), new_admin);
    
    // Clear events and accept admin transfer
    env.events().all();
    pool_client.accept_admin();
    
    let events = env.events().all();
    assert_eq!(events.len(), 1);
    assert_eq!(events[0].topic[0], Symbol::new(&env, "AdminTransferred"));
    assert_eq!(events[0].data.try_into::<Address>().unwrap(), new_admin);
}
